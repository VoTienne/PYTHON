...
name.config(font=('Verdana', 30))
name.pack(pady=10)

box1 = Text(root, width=30, height=10, font=("Verdana", 16))
box1.place(x=20, y=130)

box2 = Text(root, width=30, height=10, font=("Verdana", 16))
...
combo = ttk.Combobox(root, values=language, state="readonly", font=(
    'Verdana', 11), textvariable=selected_language)
combo.place(x=20, y=80)
combo.current(0)

# combobox 2 chọn ngôn ngữ cần dịch
selected_language_2 = StringVar()
combo2 = ttk.Combobox(root, values=language, state="readonly", font=(
    'Verdana', 11), textvariable=selected_language_2)
...
def Voice():
    with sr.Microphone() as mr:
        audio_data = r.record(mr, duration=5)
        print('Recognizing...')
        try:
...
# button chức năng
clear_button = Button(Button_frame, text='Voice', height=2, width=10, font=(
    ('Arial'), 10, 'bold'), bg='#303030', fg='#FFFFFF', command=Voice)
clear_button.place(x=500, y=400)

trans_button = Button(Button_frame, text='Dịch', height=2, width=10, font=(
    ('Arial'), 10, 'bold'), bg='#303030', fg='#FFFFFF', command=translate)
trans_button.place(x=20, y=400)

clear_button = Button(Button_frame, text='Xóa', height=2, width=10, font=(
    ('Arial'), 10, 'bold'), bg='#303030', fg='#FFFFFF', command=clear)
clear_button.place(x=140, y=400)

speach_button = Button(Button_frame, text='Đọc', height=2, width=10, font=(
    ('Arial'), 10, 'bold'), bg='#303030', fg='#FFFFFF', command=speach)
